...
|操作场景|是否回表|说明|
|---|---|---|
|`SELECT id FROM user WHERE age=25`|否|二级索引已包含 `age` 和 `id`，直接返回，无需回表|
|`SELECT username FROM user WHERE age=25`|是|`username` 不在二级索引中，需要回表|
|`SELECT * FROM user WHERE id=10`|否|直接走主键索引，无需回表|

# 回表的影响

#### 1. 性能损耗

- 回表需要两次索引查找（二级索引 + 主键索引），相比直接走主键索引，多了一次 IO 操作；
- 如果匹配的数据量很大，大量回表会显著增加数据库 IO 压力，降低查询效率。

#### 2. 优化方向：避免回表

核心思路：让二级索引包含查询所需的所有字段，即 **覆盖索引（Covering Index）**。