...
- **磁盘 I/O 更少**：非叶子节点仅存索引（关键字 + 指针），单个节点体积小，一次 I/O 可加载更多节点，减少磁盘访问次数（数据库索引的核心需求）。
- **范围查询高效**：叶子节点双向链表，无需回溯，直接遍历链表即可完成范围查询（如 “查询 id 在 10-30 之间的数据”）。
- **数据一致性**：所有数据集中在叶子节点，插入、删除仅影响叶子节点及少量索引节点，平衡维护更简单。
## B + 树的典型应用场景

### 1. 数据库索引（最核心应用）

- 关系型数据库（MySQL、Oracle）的**聚簇索引**和**非聚簇索引**均基于 B + 树实现：
    - 聚簇索引：叶子节点存储完整数据行（如 InnoDB 的主键索引），数据物理存储顺序与索引顺序一致。
    - 非聚簇索引：叶子节点存储主键（或数据地址），查询时需通过主键二次查找（回表查询）。
- 优势：适配数据库的 “磁盘存储” 特性，减少 I/O 次数，支持高效的单点查询和范围查询（如 `WHERE id=100`、`WHERE age BETWEEN 20 AND 30`）。
